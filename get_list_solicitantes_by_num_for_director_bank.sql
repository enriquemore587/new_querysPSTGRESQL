CREATE OR REPLACE FUNCTION "cw"."get_list_solicitantes_by_num_for_director_bank"(
...
BEGIN
		...
		WHERE uc.bank_id = in_bank_id 
					AND uc.user_id IN 	(
														SELECT upd.id FROM "cw"."users" AS upd
																			WHERE upd."num_client" like in_num||'%'
													)
...
SELECT cw.get_list_solicitantes_by_num_for_director_bank(5, '4');