CREATE OR REPLACE FUNCTION "cw"."get_list_solicitantes_by_rfc_curp_for_director_bank"(
		IN in_bank_id INTEGER,	
		IN "in_rfcurp" VARCHAR,
		OUT "users_list" json
)
AS $BODY$
DECLARE
		...
		WHERE uc.bank_id = in_bank_id 
					AND uc.user_id IN 	(
														SELECT upd.id FROM "cw"."user_personal_data" AS upd
																			WHERE upd."rfc" like upper(in_rfcurp)||'%'
																					OR upd."curp" like upper(in_rfcurp)||'%'
													)
...
SELECT cw.get_list_solicitantes_by_rfc_curp_for_director_bank(5, 'veae940421');