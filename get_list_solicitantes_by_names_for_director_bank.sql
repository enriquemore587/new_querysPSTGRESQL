CREATE OR REPLACE FUNCTION "cw"."get_list_solicitantes_by_names_for_director_bank"(
...
BEGIN
		...
		FOR u_row IN SELECT * FROM "cw"."user_credit_request" AS uc
		WHERE uc.bank_id = in_bank_id 
					AND uc.user_id IN 	(
														SELECT upd.id FROM "cw"."user_personal_data" AS upd
																			WHERE upd."name" like upper(in_names)||'%'
																					OR upd."last_name" like upper(in_names)||'%'
																					OR upd."last_name2" like upper(in_names)||'%'
													)
		ORDER BY uc.request_date DESC
    LOOP
...
SELECT cw.get_list_solicitantes_by_names_for_director_bank(5, 'j');